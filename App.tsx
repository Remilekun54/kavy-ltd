      ...
      <div className="relative z-10 text-center max-w-5xl px-6">
        ...
        <div className="flex justify-center gap-2 mt-12">
          {slides.map((_, index) => (
            <div
              key={index}
              className={`h-1 transition-all duration-500 ${currentSlide === index ? 'w-8 bg-[#ccff00]' : 'w-2 bg-white/20'}`}
            />
          ))}
        </div>
      </div>
  ...
  <section id={id} className="min-h-screen bg-[#f5f5f7] flex items-center justify-center py-16 md:py-24 px-6 overflow-hidden">
    <div className="max-w-7xl w-full grid grid-cols-1 lg:grid-cols-2 gap-16 lg:gap-32 items-center">
      <motion.div initial={{ opacity: 0, x: -50 }} whileInView={{ opacity: 1, x: 0 }} className="space-y-12 order-2 lg:order-1">
        ...
        <p className="text-slate-500 text-xl md:text-1xl font-light leading-relaxed italic">KAVY is not only a painting company. KAVY is a built-environment systems company designed to redefine how Africa protects and maintains its physical assets.</p>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
        {FULL_SERVICES.map((serv, i) => (
          <motion.div key={serv.id} initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.1 }} whileHover={{ y: -10 }} className="group relative p-10 bg-[#f5f5f7] rounded-[50px] border-2 border-[#ccff00] hover:border-black transition-all cursor-pointer h-[300px] md:h-[350px] flex flex-col justify-between shadow-sm overflow-hidden">
            ...
            <button onClick={onAction} className="relative z-10 text-[9px] font-black uppercase tracking-widest opacity-40 group-hover:opacity-100 flex items-center gap-2 bg-transparent border-none cursor-pointer hover:text-[#ccff00] transition-colors">Explore System <ChevronRight size={12} /></button>
          </motion.div>
        ))}
      </div>
    </div>
      ...
      <div className="max-w-7xl w-full mx-auto grid grid-cols-1 lg:grid-cols-2 gap-12 relative z-10">
        <div className="space-y-8">
          ...
          <p className="text-slate-500 text-sm md:text-lg font-light italic max-w-xl leading-snug">
            Global reach. African excellence. Initiate a mission brief with our architects.
          </p>
          ...
          <div className="flex items-center gap-4 p-4 bg-white/5 rounded-[30px] border border-white/10 group hover:border-[#ccff00] transition-all w-fit">
            ...
            <div>
              <p className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-500 italic mb-1">Executive Line</p>
              <p className="text-lg md:text-xl font-black italic">+234 814 413 0329</p>
            </div>
          </div>
        </div>
        ...
        <motion.div initial={{ opacity: 0, x: 50 }} whileInView={{ opacity: 1, x: 0 }} className="bg-white p-6 md:p-12 rounded-[50px] shadow-3xl text-black">
          ...
          {submitted ? (
            <motion.div initial={{ scale: 0.8 }} animate={{ scale: 1 }} className="flex flex-col items-center justify-center py-12 text-center">
              <CheckCircle size={60} className="text-[#ccff00] mb-4" />
              <h5 className="font-black text-xl uppercase italic">Transmission Received</h5>
              <p className="text-slate-500 text-xs mt-2 uppercase tracking-widest">Our architects will reach out shortly.</p>
              <button onClick={() => setSubmitted(false)} className="mt-8 text-[10px] font-black uppercase underline decoration-[#ccff00] decoration-2">Send another brief</button>
            </motion.div>
          ) : (
            <form onSubmit={handleSubmit} className="space-y-4">
              <div className="space-y-1">
                <label className="text-[9px] font-black uppercase tracking-widest text-slate-400 ml-4">Strategic Email</label>
                <input 
                  name="email" 
                  type="email" 
                  required 
              ...
              <div className="space-y-1">
                <label className="text-[9px] font-black uppercase tracking-widest text-slate-400 ml-4">Objective Summary</label>
                <textarea 
                  name="message" 
                  rows={2} 
                  required
              ...
              >
                {isSubmitting ? "Transmitting..." : <>Transmit Brief <Send size={14} /></>}
              </button>
            </form>
          )}
        </motion.div>
